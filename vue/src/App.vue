...
const saveData = () => {
  ...
  for (var i = 0; i < cross.length; i++) {
    ...
    a.push(cross[i].lat);
    cross_points.push(a);
    // formData.append('cross_points[][]', cross_points[i][0]);
    // formData.append('cross_points[][]', cross_points[i][1]);
  }
  // sensor节点数据
  var sensor_array = [];
  for (var i = 0; i < all_sensor.length; i++) {
    ...
    for (var j = 0; j < all_sensor[i].length; j++) {
      ...
      // formData.append('sensor_array[][][]', sensor_array[i][j][0]);
      // formData.append('sensor_array[][][]', sensor_array[i][j][1]);
    }
  ...
  for (var i = 0; i < all_gateway.length; i++) {
    ...
    for (var j = 0; j < all_gateway[i].length; j++) {
      ...
      // formData.append('gateway_array[][][]', gateway_array[i][j][0]);
      // formData.append('gateway_array[][][]', gateway_array[i][j][1]);
    }
